 ...
 * in the input array and then using this
 * information to place each element in its correct sorted position. It is
 * particularly efficient when sorting a
 * collection of integers or objects with discrete, non-negative keys.
 * Time Complexity: O(n + k) | Space Complexity: O(k) - n is the number of
 * elements in the input array. - k is the range
 * of input values (maximum value - minimum value + 1).
 * Stability: Stable
 */
